...
import { isAdministrator, listManagedUsers, LogoutButton, useCurrentUser, type AuthUser } from "../../features/auth";
...
import { formatHoursFromSeconds, formatMoneyGBP } from "../../features/payroll/format";
import { listWorkplaces, type Workplace } from "../../features/workplaces/api";

function moneyDisplay(value: string | number | null | undefined): string {
...
function percentDisplay(value: string | number | null | undefined): string {
  if (value === null || value === undefined) {
    return "—";
  }
...
export function BudgetsClient() {
  ...
  return (
    <Sheet>
      <PageHeader
        action={<LogoutButton />}
        description="Compare a planned figure to estimated labour cost from completed shifts (payroll-rounded time × profile hourly rates). Not a saved budget or accounting export."
        title="Budget calculator"
      />
      <SheetBody className="min-w-0 space-y-4 md:p-5">
        <form className="space-y-3 rounded-[var(--radius-md)] border border-[var(--color-border-dark)] bg-[var(--color-header)] p-3" onSubmit={(e) => void handleSubmit(e)}>
        ...
        {result ? (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
              <StatCard label="Planned budget" value={moneyDisplay(result.planned_budget_amount)} />
              <StatCard label="Estimated labour cost" value={moneyDisplay(result.actual_labour_cost)} />
              <StatCard label="Remaining budget" value={moneyDisplay(result.remaining_budget)} />
              <StatCard label="Over budget" value={moneyDisplay(result.over_budget_amount)} />
              <StatCard label="Budget used" value={percentDisplay(result.budget_used_percent)} />
              <StatCard
