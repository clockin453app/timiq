...
export function ClockSelfiesClient() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setLoadError("");
      try {
        const data = await listMyClockSelfies();
        if (!cancelled) {
          setItems(data);
        }
      } catch (error) {
  ...
  return (
    <Sheet>
      <PageHeader
        title="My clock selfies"
        description="Clock-in and clock-out selfies you submitted through the time clock. Images load securely after sign-in."
      />
      <SheetBody>
        {isLoading ? (
        ...
        {!isLoading && !loadError && items.length === 0 ? (
          <div className="border border-[var(--color-border)] bg-[var(--color-cell)] px-3 py-4 text-sm text-[var(--color-text-muted)]">
            No clock selfies found yet. Complete a clock-in or clock-out with a selfie to see them here.
          </div>
        ) : null}
