...
from app.modules.payroll.service import export_pdf_report, export_xlsx_report
...
@patch("app.modules.payroll.router.export_xlsx_report")
def test_export_xlsx_range_response_passes_date_and_employee_filter(
    mock_export: MagicMock,
    client: TestClient,
) -> None:
    company_id = uuid.uuid4()
    employee_id = uuid.uuid4()
    admin = _user(role=SystemRole.ADMIN, company_id=company_id)
    mock_export.return_value = b"PK\x03\x04 payroll"
    ...
    try:
        ...
        disposition = response.headers.get("content-disposition", "")
        assert "timiq-payroll-report-2025-01-07-to-2025-01-08.xlsx" in disposition
        kwargs = mock_export.call_args.kwargs
        assert kwargs["date_from"] == date(2025, 1, 7)
        assert kwargs["date_to"] == date(2025, 1, 8)
        assert kwargs["employee_user_id"] == employee_id
...
@patch("app.modules.payroll.router.export_xlsx_report")
def test_export_xlsx_week_response_uses_range_filename(
    mock_export: MagicMock,
    client: TestClient,
) -> None:
    company_id = uuid.uuid4()
    ...
    try:
        response = client.get(
            f"/api/payroll/export.xlsx?company_id={company_id}&week_start=2025-01-06",
        )
        assert response.status_code == 200
        disposition = response.headers.get("content-disposition", "")
        assert "timiq-payroll-report-2025-01-06-to-2025-01-12.xlsx" in disposition
    finally:
        app.dependency_overrides.clear()
...
def test_export_xlsx_range_rejects_invalid_range(client: TestClient) -> None:
    company_id = uuid.uuid4()
    admin = _user(role=SystemRole.ADMIN, company_id=company_id)
    app.dependency_overrides[require_admin_or_administrator] = lambda: admin
    try:
        response = client.get(
            f"/api/payroll/export.xlsx?company_id={company_id}&date_from=2025-01-09&date_to=2025-01-08",
        )
        assert response.status_code == 400
    finally:
        app.dependency_overrides.clear()
...
def test_employee_cannot_access_payroll_export_xlsx(client: TestClient) -> None:
    employee = _user(role=SystemRole.EMPLOYEE, company_id=uuid.uuid4())
    app.dependency_overrides[require_admin_or_administrator] = lambda: employee
    try:
        response = client.get(
            f"/api/payroll/export.xlsx?company_id={employee.company_id}&week_start=2025-01-06",
        )
        assert response.status_code == 403
    finally:
        app.dependency_overrides.clear()
...
@patch("app.modules.payroll.service.get_company_by_id")
@patch("app.modules.payroll.service.create_internal_audit_event")
def test_export_xlsx_report_company_admin_own_company(
    mock_audit: object,
    mock_company: object,
    mock_report: object,
) -> None:
    ...
    assert sheet["A1"].value == "TimIQ Payroll Report"
    assert sheet["B3"].value == "Own Co"
