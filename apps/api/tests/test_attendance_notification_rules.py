...
from app.modules.attendance_notifications.service import (
    _forgot_clock_out_dedupe_key,
    _missing_clock_in_dedupe_key,
...
def _run_with_settings(
    ...
    *,
    ...
    with (
        ...
        patch("app.modules.attendance_notifications.service.user_has_clock_in_between", return_value=has_clock_in),
        ...
        result = run_attendance_notification_check_once(
            MagicMock(),
            now_utc=now_utc or datetime(2026, 5, 18, 10, 0, tzinfo=timezone.utc),
        )
    return created, result, seen_calls, employee, admin


def test_missing_clock_in_dedupe_key_includes_recipient_and_workday() -> None:
    company_id = uuid.uuid4()
    employee_id = uuid.uuid4()
    recipient_id = uuid.uuid4()
    key = _missing_clock_in_dedupe_key(
        company_id=company_id,
        employee_id=employee_id,
        work_date=date(2026, 5, 18),
        recipient_id=recipient_id,
    )
    assert key == f"attendance:missing_clock_in:{company_id}:{employee_id}:2026-05-18:{recipient_id}"


def test_forgot_clock_out_dedupe_key_is_employee_workday_scoped() -> None:
    company_id = uuid.uuid4()
    employee_id = uuid.uuid4()
    recipient_id = uuid.uuid4()
    ...
    assert key == f"attendance:forgot_clock_out:{company_id}:{employee_id}:2026-05-17:{recipient_id}"


def test_late_arrival_creates_canonical_missing_clock_in() -> None:
    settings = _base_settings(uuid.uuid4())
    settings.late_arrival_enabled = True
    created, result, _seen, employee, admin = _run_with_settings(settings)

    assert result.notifications_created == 2
    assert {row["kind"] for row in created} == {"attendance_missing_clock_in"}
    assert {row["recipient_user_id"] for row in created} == {employee.id, admin.id}
    for row in created:
        assert row["dedupe_key"] == _missing_clock_in_dedupe_key(
            company_id=settings.company_id,
...
def test_forgot_clock_in_creates_canonical_missing_clock_in() -> None:
    settings = _base_settings(uuid.uuid4())
    settings.forgot_clock_in_enabled = True
    created, result, _seen, employee, _admin = _run_with_settings(settings)

    assert result.notifications_created == 2
    assert {row["kind"] for row in created} == {"attendance_missing_clock_in"}
    assert all("missing_clock_in" in row["dedupe_key"] for row in created)
    assert employee.id in {row["subject_user_id"] for row in created}


def test_late_and_forgot_in_do_not_overlap_for_same_employee_day() -> None:
    settings = _base_settings(uuid.uuid4())
    settings.late_arrival_enabled = True
    settings.forgot_clock_in_enabled = True
    created, result, _seen, employee, admin = _run_with_settings(settings)
    ...
    assert {row["kind"] for row in created} == {"attendance_missing_clock_in"}
    assert {row["recipient_user_id"] for row in created} == {employee.id, admin.id}
    keys = {row["dedupe_key"] for row in created}
    assert len(keys) == 2


def test_repeated_check_reuses_dedupe_without_extra_inserts() -> None:
    settings = _base_settings(uuid.uuid4())
    settings.late_arrival_enabled = True
    created_once: list[dict] = []

    def create_record(_db, **kwargs):
        created_once.append(kwargs)
        return False
    ...
    with (
        ...
        patch("app.modules.attendance_notifications.service.has_approved_leave_on_date", return_value=False),
...
def test_clock_in_resolves_missing_clock_in_alerts() -> None:
    settings = _base_settings(uuid.uuid4())
    settings.late_arrival_enabled = True
    created, result, seen_calls, employee, _admin = _run_with_settings(settings, has_clock_in=True)

    assert created == []
    assert result.notifications_created == 0
    assert len(seen_calls) == 1
    assert seen_calls[0]["subject_user_id"] == employee.id
    assert seen_calls[0]["work_date"] == date(2026, 5, 18)
...
def test_forgot_clock_out_creates_internal_notification() -> None:
    ...
    created, result, _seen, _employee, admin = _run_with_settings(
        settings,
        employee=employee,
        open_shift=[(shift, employee, None)],
    )

    assert result.notifications_created == 2
    assert {row["kind"] for row in created} == {"attendance_forgot_clock_out"}
    for row in created:
        assert row["dedupe_key"] == _forgot_clock_out_dedupe_key(
            company_id=company_id,
            employee_id=employee.id,
            work_date=date(2026, 5, 17),
            recipient_id=row["recipient_user_id"],
...
def test_notify_employee_false_suppresses_employee_target() -> None:
    ...
    created, _result, _seen, _e, _a = _run_with_settings(settings, employee=employee, admin=admin)
...
def test_approved_leave_suppresses_late_and_forgot_in_when_enabled() -> None:
    ...
    created, result, _seen, _e, _a = _run_with_settings(settings, approved_leave=True)
