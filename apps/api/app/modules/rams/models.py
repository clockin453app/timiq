...
class RamsAssessment(Base):
    ...
    source_type = mapped_column(String(32), nullable=False, default="template", index=True)
    uploaded_pdf_original_filename = mapped_column(String(255), nullable=True)
    uploaded_pdf_storage_path = mapped_column(String(500), nullable=True)
    uploaded_pdf_content_type = mapped_column(String(120), nullable=True)
    uploaded_pdf_file_size_bytes = mapped_column(Integer, nullable=True)
    uploaded_pdf_checksum_sha256 = mapped_column(String(64), nullable=True)
    uploaded_pdf_version = mapped_column(Integer, nullable=False, default=1)
    uploaded_pdf_uploaded_by_user_id = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("users.id", ondelete="SET NULL"),
        nullable=True,
    )
    uploaded_pdf_uploaded_at = mapped_column(DateTime(timezone=True), nullable=True)
